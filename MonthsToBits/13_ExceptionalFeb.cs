...
{
    public class ExceptionalFeb : IDaysInMonth
    {
        ILongMonth longMonth;

        public ExceptionalFeb()
        {
            longMonth = new SetMembership();
        }

        public int DaysInMonth(int month, int year)
        {
            return month == 2 ? 28 + LeapYear.DayOffset(year) : 30 + (longMonth.IsLongMonth(month) ? 1 : 0);
        }
    }

    public class DExceptionalFebTests
    {
        [Theory]
        [InlineData(1, 2000)]
        [InlineData(3, 2002)]
